...
public class UrlNavigation {
    ...
    private FileUploadIntentsCreator creator;
    private ActivityResultLauncher<Intent> captureIntentLauncher;
    ...
    UrlNavigation(MainActivity activity) {
        ...
        this.creator = new FileUploadIntentsCreator(mainActivity);
        initializeCaptureIntentLauncher();
    }
...
//        Log.d(TAG, "intercept " + url);
    ...
    private void initializeCaptureIntentLauncher() {
        this.captureIntentLauncher = mainActivity.registerForActivityResult(
                ...
                result -> {
                    ...
                    if (data != null && data.getClipData() != null) {
                        ...
                        if (uploadCallback != null) {
                            uploadCallback.onReceiveValue(files.toArray(new Uri[files.size()]));
                            uploadCallback = null;
                        }
                    ...
                    // from camera
                    if (this.creator.getCurrentCaptureUri() != null) {

                        if (uploadCallback != null) {
                            uploadCallback.onReceiveValue(new Uri[]{this.creator.getCurrentCaptureUri()});
                            uploadCallback = null;
                        }
    ...
    public void cancelFileUpload() {
        ...
        this.creator.deleteUriFiles();
    }

    public void setUploadCallback(ValueCallback<Uri[]> uploadCallback) {
        this.uploadCallback = uploadCallback;
    }

    public boolean chooseFileUpload(final String[] mimetypespec, final boolean multiple) {
        creator.setUploadSpecs(mimetypespec, multiple);

        if ((creator.canUploadImage() || creator.canUploadVideo())) {
            ...
            // Add WRITE_EXTERNAL_STORAGE permission if needed
            if (creator.needsStoragePermissionAndroid9()) {
                permissionsToRequest.add(Manifest.permission.WRITE_EXTERNAL_STORAGE);
            }

            mainActivity.getPermission(permissionsToRequest.toArray(new String[0]), (permissions, grantResults) -> {
                if (!verifyPermissionResults(permissions, grantResults)) return;
                launchChooserIntent();
            });
            return true;
        }
    ...
    private boolean launchChooserIntent() {
        Intent intentToSend = creator.chooserIntent();

        try {
            captureIntentLauncher.launch(intentToSend);
            return true;
    ...
    public boolean openDirectCamera(final String[] mimetypespec, final boolean multiple) {
        ...
        // Add CAMERA permission if required
        if (!Utils.isPermissionGranted(mainActivity, Manifest.permission.CAMERA)) {
            permissionsToRequest.add(Manifest.permission.CAMERA);
        }
    ...
    private boolean openDirectCameraAfterPermission(String[] mimetypespec, boolean multiple) {
        ...
        this.creator.setUploadSpecs(mimetypespec, multiple);
    ...
    private boolean verifyPermissionResults(String[] permissions, int[] grantResults) {
        for (int i = 0; i < permissions.length; i++) {
            String permission = permissions[i];
            int result = grantResults[i];

            if (Objects.equals(permission, Manifest.permission.CAMERA) && result == -1) {
                Toast.makeText(mainActivity, R.string.upload_camera_permission_denied, Toast.LENGTH_SHORT).show();
                this.cancelFileUpload();
                return false;
            }
            ...
            if (Objects.equals(permission, Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                if (result == -1) {
                    // If write permission got denied on devices Android 9 or lower,
                    // save file to internal app storage instead
                    creator.setForceSaveToInternalStorage(true);
                } else if (result == 0) {
                    // Reset forceSaveToInternalStorage if granted
                    creator.setForceSaveToInternalStorage(false);
                }
            }
        }
        return true;
