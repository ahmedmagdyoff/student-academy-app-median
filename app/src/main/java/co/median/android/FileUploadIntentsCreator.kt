...
class FileUploadIntentsCreator(val context: Context) {
    ...
    private fun videoCameraIntents(): ArrayList<Intent> {
        ...
        for (resolve in resolveList) {
            ...
            intent.component = ComponentName(resolve.activityInfo.packageName, resolve.activityInfo.name)
            intent.setPackage(packageName)
            applyVideoCameraSettings(intent, packageName)
            intents.add(intent)
        }
    ...
    private fun applyVideoCameraSettings(videoIntent: Intent, packageName: String) {
        ...
        // Skip adding EXTRA_OUTPUT if saveToGallery is true for Video Recording;
        // the file will be automatically saved to DCIM/Camera folder and appear in the gallery.
        if (appConfig.cameraConfig.saveToGallery && isAndroid10orAbove && !forceSaveToInternalStorage) {
            return
        }
        ...
        if (appConfig.cameraConfig.saveToGallery && canSaveToPublicStorageAndroid9orBelow && !forceSaveToInternalStorage) {
            return
        }

        if (packageName.equals("com.sec.android.app.camera", ignoreCase = true)) {
            // On Samsung devices, the default Camera app (`com.sec.android.app.camera`) does not honor MediaStore.EXTRA_OUTPUT.
            // Setting it may result in no data being returned.
            // The video will always be saved in the default external location based on the Camera app's settings.
            return
        }
